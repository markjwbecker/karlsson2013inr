...
where $x_t$ is a vector of $d$ deterministic variables (constant and or dummy/time trend), and
...
is a $k \times m$ matrix. We have normally distributed errors $u_t \sim N(0, \Psi)$.
...
based on the Minnesota prior with overall tightness $\pi_1$, relative (cross-equation) tightness $\pi_2$ and lag decay rate $\pi_3$. We also have a hyperparameter $\pi_4$ for the deterministic terms. For the normal-diffuse prior, we use Jeffreys' prior for $\Psi$
...
An uninformative prior can be (default in this package) specified by setting $\underline{S}=(\underline{v}-k-1)\hat{\Psi}$ where $\hat{\Psi}$ is the least squares estimate from the VAR($p$) (including the constant and dummy/trend variable if applicable), and $\underline{v}=m+2$. You may however change $\underline{S}$ and $\underline{v}$ as you please.


```{r, fig.width=10, fig.height=10,eval=TRUE, message=FALSE, warning=FALSE}
rm(list = ls())
...
is a $dp$-dimensional vector of lagged deterministic (exogenous) variables, and
...
Here $\pi (\Gamma_d)$ is based on the Minnesota prior with overall tightness $\pi_1$, relative (cross-equation) tightness $\pi_2$ and lag decay rate $\pi_3$. Note that $\pi (\Lambda)$ is the core of the steady-state model, where we specify our prior beliefs about the location and scale of the steady-state parameters. For $\Psi$ we use Jeffreys' prior

$$
p(\Psi) \propto\left|\Psi \right|^{-(m+1)/2}
$$

Alternatively a proper inverse Wishart, $\Psi \sim iW(\underline{S}, \underline{v})$, for $\Psi$ can be used. An uninformative inverse Wishart prior can again be specified by setting $\underline{S}=(\underline{v}-k-1)\hat{\Psi}$ where $\hat{\Psi}$ is the least squares estimate from the VAR($p$) (including the constant and dummy/trend variable if applicable), and $\underline{v}=m+2$. Again, the scale matrix and degrees of freedom may be changed to the users desires.


```{r, fig.width=10, fig.height=8,eval=TRUE, message=FALSE, warning=FALSE}
rm(list = ls())
par(mfcol = c(1, 1))
