...
An uninformative prior can be (and is in this package) specified by setting $\underline{S}=(m_0-k-1)\hat{\Psi}$ where $\hat{\Psi}$ is the least squares estimate from the VAR($p$) (including the constant and dummy/trend variable if applicable), and $\underline{v}=m+2$.
